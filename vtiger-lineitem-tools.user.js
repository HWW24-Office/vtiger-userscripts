...
(async function () {
  ...
  const SNReconcile = (function () {
    ...
    function injectPanel() {
      ...
      panel.innerHTML = `
        ...
        <div class="panel-body">
          <div class="section">
            <div class="section-title">Vollständige Soll-Liste (kompletter Abgleich)</div>
            ...
            <div class="btn-row" style="margin-top:8px">
              <button id="hw24-sn-reconcile" class="btn btn-primary" style="flex:2">🔍 Voll abgleichen</button>
              <button id="hw24-sn-refresh" class="btn btn-secondary">🔄</button>
            </div>
          </div>
          <div class="section">
            <div class="section-title">Stückweise Änderung (Rest bleibt unverändert)</div>
            <textarea id="hw24-sn-add" placeholder="Nur hinzufügen: diese Seriennummern neu aufnehmen"></textarea>
            <textarea id="hw24-sn-remove" placeholder="Nur entfernen: diese Seriennummern aus Angebot/SO entfernen"></textarea>
            <div class="btn-row" style="margin-top:8px">
              <button id="hw24-sn-delta" class="btn btn-secondary" style="flex:2">➕➖ Delta abgleichen</button>
              <button id="hw24-sn-add-only" class="btn btn-secondary">➕ Nur Add</button>
              <button id="hw24-sn-remove-only" class="btn btn-secondary">➖ Nur Remove</button>
            </div>
          </div>
      ...
      $('hw24-sn-reconcile').onclick = () => performReconcile('full');
      $('hw24-sn-delta').onclick = () => performReconcile('delta');
      $('hw24-sn-add-only').onclick = () => performReconcile('add');
      $('hw24-sn-remove-only').onclick = () => performReconcile('remove');
      $('hw24-sn-refresh').onclick = () => { reconcileResult = null; performReconcile('full'); };
    ...
    function togglePanel(forceState) {
      ...
      if (panel) {
        ...
        if (panelVisible) { reconcileResult = null; performReconcile('full'); }
      }
    }

    function buildIstIndex(items) {
    ...
    function buildPositionsToDelete(items, toRemoveSet) {
      const positionsToDelete = [];
      items.forEach(it => {
        if (it.sns.length === 0) return;
        const remainingSNs = it.sns.filter(sn => !toRemoveSet.has(sn));
        if (remainingSNs.length === 0 && it.sns.some(sn => toRemoveSet.has(sn))) {
          const meta = it.meta || {};
    ...
    function collectInputLists(mode) {
      const fullList = parseList($('hw24-sn-soll')?.value);
      const addList = parseList($('hw24-sn-add')?.value);
      const removeList = parseList($('hw24-sn-remove')?.value);

      if (mode === 'full') return { mode, fullList, addList: [], removeList: [] };
      if (mode === 'add') return { mode, fullList: [], addList, removeList: [] };
      if (mode === 'remove') return { mode, fullList: [], addList: [], removeList };
      return { mode: 'delta', fullList: [], addList, removeList };
    }

    function computeReconcileResult(input, items, istIndex) {
      const istSNs = new Set(istIndex.keys());
      const matching = [];
      const toRemove = [];
      const toAdd = [];
      const alreadyPresent = [];
      const notFoundForRemove = [];
      const conflicts = [];

      if (input.mode === 'full') {
        const fullSet = new Set(input.fullList);
        for (const [sn, info] of istIndex) {
          if (fullSet.has(sn)) matching.push({ sn, position: info.position, rn: info.item.rn });
          else toRemove.push({ sn, position: info.position, rn: info.item.rn });
        }
        for (const sn of input.fullList) {
          if (!istSNs.has(sn)) toAdd.push({ sn });
        }
      } else {
        const addSet = new Set(input.addList);
        const removeSet = new Set(input.removeList);
        const conflictSet = new Set([...addSet].filter(sn => removeSet.has(sn)));

        for (const sn of conflictSet) conflicts.push({ sn });

        for (const sn of addSet) {
          if (conflictSet.has(sn)) continue;
          if (istSNs.has(sn)) {
            const info = istIndex.get(sn);
            alreadyPresent.push({ sn, position: info?.position || '—' });
          } else {
            toAdd.push({ sn });
          }
        }

        for (const sn of removeSet) {
          if (conflictSet.has(sn)) continue;
          if (istSNs.has(sn)) {
            const info = istIndex.get(sn);
            toRemove.push({ sn, position: info?.position || '—', rn: info?.item?.rn || '' });
          } else {
            notFoundForRemove.push({ sn });
          }
        }
      }

      const toRemoveSet = new Set(toRemove.map(r => r.sn));
      const positionsToDelete = buildPositionsToDelete(items, toRemoveSet);
      ...
      return {
        ...
        positionsToDelete,
        inputCounts: {
          full: input.fullList.length,
          add: input.addList.length,
          remove: input.removeList.length
        }
      };
    }

    function renderReconcileResults(result, istTotalCount) {
      const resultsContainer = $('hw24-sn-results');
      const actionsContainer = $('hw24-sn-actions');
      if (!resultsContainer) return;

      const { mode, matching, toRemove, toAdd, alreadyPresent, notFoundForRemove, conflicts, positionsToDelete, inputCounts } = result;

      resultsContainer.style.display = 'block';
      actionsContainer.style.display = (toRemove.length > 0 || toAdd.length > 0) ? 'flex' : 'none';

      if (mode === 'full' && inputCounts.full === 0) {
        resultsContainer.innerHTML = `
          <div class="summary-box">
            <div style="font-weight:600;margin-bottom:8px;">Aktueller Stand</div>
            <div class="summary-row"><span>Positionen:</span><span>${result.items.length}</span></div>
            <div class="summary-row"><span>Seriennummern gesamt:</span><span>${istTotalCount}</span></div>
          </div>
          <div style="color:#64748b;font-size:12px;text-align:center;padding:10px;">Füge die Soll-Liste vom Kunden ein und klicke "Voll abgleichen"</div>
        `;
        actionsContainer.style.display = 'none';
        return;
      }

      if (mode !== 'full' && inputCounts.add === 0 && inputCounts.remove === 0) {
        resultsContainer.innerHTML = `
          <div class="summary-box">
            <div style="font-weight:600;margin-bottom:8px;">Delta-Modus</div>
            <div class="summary-row"><span>Seriennummern gesamt (Ist):</span><span>${istTotalCount}</span></div>
          </div>
          <div style="color:#64748b;font-size:12px;text-align:center;padding:10px;">Füge SNs in "Nur hinzufügen" und/oder "Nur entfernen" ein und starte den Delta-Abgleich.</div>
        `;
        actionsContainer.style.display = 'none';
        return;
      }
      ...
      let html = `
        <div class="summary-box">
          <div class="summary-row total"><span>Modus:</span><span>${title}</span></div>
          ${mode === 'full' ? `<div class="summary-row"><span>✓ Übereinstimmend:</span><span style="color:#166534">${matching.length}</span></div>` : ''}
          <div class="summary-row"><span>✗ Zu entfernen (nicht in Soll):</span><span style="color:#991b1b">${toRemove.length}</span></div>
          <div class="summary-row"><span>➕ Hinzuzufügen:</span><span style="color:#92400e">${toAdd.length}</span></div>
          ${alreadyPresent.length > 0 ? `<div class="summary-row"><span>ℹ Bereits vorhanden:</span><span style="color:#166534">${alreadyPresent.length}</span></div>` : ''}
          ${notFoundForRemove.length > 0 ? `<div class="summary-row"><span>ℹ Nicht gefunden (Remove):</span><span style="color:#64748b">${notFoundForRemove.length}</span></div>` : ''}
          ${conflicts.length > 0 ? `<div class="summary-row"><span>⚠ Konflikt (Add+Remove):</span><span style="color:#92400e">${conflicts.length}</span></div>` : ''}
          ...
          ${mode === 'full' ? `<div class="summary-row total"><span>Soll-Liste:</span><span>${inputCounts.full} SNs</span></div>` : `<div class="summary-row total"><span>Input:</span><span>+${inputCounts.add} / -${inputCounts.remove}</span></div>`}
        </div>
      `;
      ...
      if (toAdd.length > 0) {
        html += `<div class="result-group"><div class="result-header missing"><span>➕ Werden hinzugefügt</span><span class="count">${toAdd.length}</span></div><div class="result-sns">${toAdd.map(m => `<span class="result-sn missing">${m.sn}</span>`).join('')}</div><div style="font-size:11px;color:#92400e;margin-top:6px;padding-left:8px;">Diese SNs müssen einer Position zugeordnet werden.</div></div>`;
      }
      if (alreadyPresent.length > 0) {
        html += `<div class="result-group"><div class="result-header matching"><span>ℹ Bereits vorhanden</span><span class="count">${alreadyPresent.length}</span></div><div class="result-sns">${alreadyPresent.map(m => `<span class="result-sn matching">${m.sn}<span class="result-position">${m.position}</span></span>`).join('')}</div></div>`;
      }
      if (notFoundForRemove.length > 0) {
        html += `<div class="result-group"><div class="result-header" style="background:#e2e8f0;color:#334155;"><span>ℹ Nicht gefunden (Remove)</span><span class="count">${notFoundForRemove.length}</span></div><div class="result-sns">${notFoundForRemove.map(m => `<span class="result-sn" style="background:#e2e8f0;color:#334155;">${m.sn}</span>`).join('')}</div></div>`;
      }
      if (conflicts.length > 0) {
        html += `<div class="result-group"><div class="result-header missing"><span>⚠ Konflikt: Add und Remove</span><span class="count">${conflicts.length}</span></div><div class="result-sns">${conflicts.map(m => `<span class="result-sn missing">${m.sn}</span>`).join('')}</div><div style="font-size:11px;color:#92400e;margin-top:6px;padding-left:8px;">Diese SNs stehen gleichzeitig in Add und Remove und wurden nicht automatisch verarbeitet.</div></div>`;
      }
    ...
    async function performReconcile(mode = 'full') {
      const input = collectInputLists(mode);
      ...
      const istIndex = buildIstIndex(items);
      reconcileResult = computeReconcileResult(input, items, istIndex);
      renderReconcileResults(reconcileResult, istIndex.size);
    }
    ...
    async function applyChanges() {
      ...
      const missingSNs = reconcileResult.toAdd.map(m => m.sn);
      ...
      const writeBack = () => {
        ...
        if (missingSNs.length > 0) msg.push(`${missingSNs.length} SN(s) hinzugefügt/zugeordnet`);
  ...
  const EMAILMakerTools = (function () {
    ...
    function resetStep1LanguageState(scope = document) {
      const containers = scope.querySelectorAll?.('.SendEmailFormStep1, #sendEmailFormStep1, .modelContainer, .modal, [role="dialog"]') || [];
      for (const container of containers) {
        delete container.dataset.hw24LangAutoDone;
        delete container.dataset.hw24LangManualOverride;
        delete container.dataset.hw24LangUserValue;
        delete container.dataset.hw24LangApplied;
      }
    }
